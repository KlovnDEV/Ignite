...
------ l o c a l e s ------ local noCar = "No car nearby" local text = "~r~lockpick~w~ door?" -- lockpick the door 
local textUnlock = "~g~[E]~w~ Enter" -- enter the house local insideText = "~g~[E]~w~ Exit" -- exit the door local 
abortConfirm = "You have aborted the lockpicking" local searchText = "~g~[E]~w~ Search" -- search the spot local 
emptyMessage = "There is nothing here!" -- if you press E where it is empty local emptyMessage3D = "~r~Empty" -- if 
the spot is empty local closetText = "~g~[E]~w~ Peek into closet" -- text at closet local abortLock = "~g~[E]~w~ To 
abort lockpicking" local noLockpickText = "You don't have any lockpick!" -- if you don't have a lockpick and you 
try to do the burglary local carUnlocked = "You have unlocked the car" local youFound = "From the" -- when you 
steal something local burglaryDetected = "A burglary has been detected at" -- text 1 cops gets sent local sentPhoto 
= "We've sent you a photo of the criminal." -- if you use qalle's camerasystem this will be in the message too 
local item = {'black_money', 'bread', 'lockpick', 'water'} local exitPos = {pos = {x = 0, y = 0, z = 0, h = 0 }} 
local lastDoor = 0 local noiseXYZ = { x = 346.53 , y = -1003.44 , z = -99.2} ---------------------------
...
RegisterNetEvent('esx_burg:onUse')
AddEventHandler('esx_burg:onUse', function()
  ...
  local coords		= GetEntityCoords(playerPed)
    ...
    if DoesEntityExist(vehicle) then
      ...
      if randi == 1 then
      TriggerServerEvent('esx_burg:removeKit')
      end
...
RegisterNetEvent('esx_burg:Lockpick')
AddEventHandler('esx_burg:Lockpick', function(xPlayer)
  lockpicking = true
  Citizen.Wait(100)
  lockpicking = false
end)
...
function steal(k)
  ...
  Citizen.Wait(2000)
  procent(50)
  TriggerServerEvent('esx_burg:Add', goods, 1)
...
function HouseBreak(house)
  ...
  if rand == 1 then
    TriggerServerEvent('esx_burg:Remove', 'lockpick', 1)
  end  
  fade()
  ClearPedTasks(playerPed)
  FreezeEntityPosition(playerPed, false)
...
RegisterNetEvent('esx_burg:Sound')
AddEventHandler('esx_burg:Sound', function(sound1, sound2)
  ...
  ESX.UI.Menu.Open(
      ...
      function(data, menu)
          if data.current.value == 'ring' then
              TriggerServerEvent('esx_burg:sellring')
          elseif data.current.value == 'rolex' then
              TriggerServerEvent('esx_burg:sellrolex')
          elseif data.current.value == 'camera' then
              TriggerServerEvent('esx_burg:sellcamera')
          elseif data.current.value == 'goldNecklace' then
              TriggerServerEvent('esx_burg:sellgoldNecklace')
          elseif data.current.value == 'laptop' then
              TriggerServerEvent('esx_burg:selllaptop')
          elseif data.current.value == 'samsungS10' then
              TriggerServerEvent('esx_burg:sellsamsungS10')
          end
      end,
      function(data, menu)
          menu.close()
