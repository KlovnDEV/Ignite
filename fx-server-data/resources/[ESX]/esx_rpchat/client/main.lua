...
AddEventHandler('esx_rpchat:sendProximityMessage', function(id, prefix, message, color)
	...
	if target == _source then
		TriggerEvent('chatMessage', prefix, message, color)
...
Citizen.CreateThread(function()
	...
	TriggerEvent('chat:addSuggestion', '/twitter',  _U('twt_help'),  {{name=_U('ooc_argument_name'), help=_U('ooc_argument_help')}})
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		...
		TriggerEvent('chat:removeSuggestion', '/twitter')
