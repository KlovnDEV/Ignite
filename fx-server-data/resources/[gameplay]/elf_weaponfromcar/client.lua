...
local txt = "The individual" -- /me text
local bwtxt = "* pulls a weapon from the trunk of the car *"
local dbtxt = "* takes a weapon out of the bag *"
local dbtxterr = "You're going to need to wear a bag!"
local bwtxterr = "You need to take this out of the boot of the car.."
local swtxt = "* pulls out a weapon *"
local mwtxt = "* pulls out a melee weapon *"



-- 3dtext settings --

local color = { r = 255, g = 0, b = 0, alpha = 100 } -- color of the text 
					...
					if GetVehiclePedIsIn(playerPed, false) == 0 and DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and bigWeaponOut == false then
						...
						if duffle == true then
                            if --[[IsPedModel(playerPed,1885233650) and]] bigWeaponOut == false and GetVehiclePedIsIn(playerPed, false) == 0 then -- male
                                ...
                                if GetPedDrawableVariation(playerPed,5) == 44 or GetPedDrawableVariation(playerPed,5) == 45  then
                                    bigWeaponOut = true
                                    if tafdb == true then
                                        local text = dbtxt
                                        TriggerServerEvent('3dme:shareDisplay', text)
